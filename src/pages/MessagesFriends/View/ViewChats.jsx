...
const ViewChats = ({ chatsUserRefined }) => {
  ...
  const getCurrentChat = () => {
    const newChatCurrent =  chatsUserRefined.filter(({ chat_id }) => chat_id === chat_id_user)[0];
    ...
    if (newChatCurrent && chat_id_user) {
      ...
      setView('chatNotFound')
    }
  };

  const avatarChatCurrent = chatCurrent?.users.filter((user) => user.nickname !== dataUser.nickname)[0]?.avatar_url || "";

  return (
    <div className="grid h-full max-w-[1111px] sm:gap-x-6 lg:gap-x-0 xl:gap-x-6 grid-rows-[auto_auto_740px] sm:grid-rows-[2fr_3fr] grid-cols-[218px_1fr] lg:grid-cols-1 lg:grid-rows-[auto_auto_740px] xl:grid-cols-[218px_1fr] xl:grid-rows-[2fr_3fr]">
